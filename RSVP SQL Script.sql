...
-- Highest number of movies were released in year 2017. 
-- Most of the movies are released in the month of March.
...
SELECT 
    year, COUNT(DISTINCT id) AS India_USA_2019
FROM
    movie
WHERE
    (country LIKE '%India%'
...
-- HIghest number of movies belong to Drama genre followed by Comedy and Thriller.
...
-- 3289 movies out of the dataset belong to only one genre
...
-- Movies belonging to "Action" genre has highest average duration of 112.88 mins while horror movies with least average duration of 92.72 mins. 
...
-- Drama genre and Family genre have highest and lowest releases respectively.
...
-- 361 movies were given a median reating of 8 during this period.
...
-- Type your code below:
SELECT 
    name, COUNT(name) AS movie_count
FROM
    names n
        INNER JOIN
...
(	SELECT g.genre, year, title AS movie_name,
		   CASE
			 WHEN worlwide_gross_income LIKE 'INR%' 
				THEN Cast(Replace(worlwide_gross_income, 'INR', '') AS DECIMAL(12)) / 82
...
-- Assumption of 1 USD = INR 82 has been made. 
-- Star Wars: Episode VIII - The Last Jedi in the year 2017, Avengers: Infinity War in the year 2018 and Avengers: Endgame in the year 2019 emerged as the highest grossing movies considering the top 3 genres. 
...
-- Star Cinema and Twentieth Century Fox are revealed as the top production houses considering multilingual movies.
...
-- Parvathy Thiruvothu, Susan Brown and Amanda Lawrence have emerged as the top actress in the drama genre considering the number of super hit movies. 
...
(
           SELECT     d.name_id,
                      ...
                      total_votes,
                      m.date_published,
                      Lead (date_published, 1) OVER (partition BY d.name_id ORDER BY date_published, movie_id) AS successor_date
...
(
       SELECT *,
              Datediff(successor_date, date_published) AS date_diff
...
-- with the above analysis, Andrew Jones has been identified as the top director considering number of movies produced. However, the min and max ratings of Steven Soderbergh are quite exceptional. 
